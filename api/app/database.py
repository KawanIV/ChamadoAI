...
async def set_tenant_context(session: AsyncSession, tenant_id: str) -> None:
    ...
    await session.execute(__import__("sqlalchemy").text("select set_config('app.platform_admin', 'false', true)"))
...
async def set_platform_context(session: AsyncSession) -> None:
    await session.execute(__import__("sqlalchemy").text("select set_config('app.platform_admin', 'true', true)"))
