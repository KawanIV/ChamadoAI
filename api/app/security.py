...
class Principal(BaseModel): user_id:str; tenant_id:str; role:str; area_id:str|None=None
...
def create_access_token(p:Principal)->str:
    now=datetime.now(timezone.utc);return jwt.encode({"sub":p.user_id,"tenant_id":p.tenant_id,"role":p.role,"area_id":p.area_id,"iat":now,"exp":now+timedelta(minutes=30)},get_settings().jwt_secret,algorithm="HS256")
def decode_access_token(token:str)->Principal:
    try:data=jwt.decode(token,get_settings().jwt_secret,algorithms=["HS256"],options={"require":["sub","tenant_id","role","exp"]});return Principal(user_id=data["sub"],tenant_id=data["tenant_id"],role=data["role"],area_id=data.get("area_id"))
...
def require_admin(p:Principal=Depends(current_principal))->Principal:
    if p.role!="platform_admin":raise HTTPException(status.HTTP_403_FORBIDDEN,"Permissão insuficiente")
    return p
def require_company_admin(p:Principal=Depends(current_principal))->Principal:
    if p.role!="company_admin":raise HTTPException(status.HTTP_403_FORBIDDEN,"Permissão exclusiva da empresa")
    return p
def require_agent(p:Principal=Depends(current_principal))->Principal:
    if p.role not in {"company_admin","agent"}:raise HTTPException(status.HTTP_403_FORBIDDEN,"A gestão de chamados é exclusiva da empresa e dos prestadores")
    return p
